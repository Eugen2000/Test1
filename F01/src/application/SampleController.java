...
//--------------------------------------------------------------------------
	...
	private double dotSize=40;
	private double dotSpace=5;
	private Color  defaultDotBodyColor=Color.RED;
	private Color  defaultDotBorderColor=Color.BLACK;
	private Color defaultBG=Color.rgb(0, 255, 204);
			...
			for(int jk=0;jk<=2;jk++){
				...
				/*	
					...
					*/
					...
					myDots[pX][pY].neighbors++;
					CanvFX.getGraphicsContext2D().setFill(defaultBG);
...
*/				}												
	...
	private void NextGen(){
		for(int i=0;i<numDotX;i++){
			for(int j=0;j<numDotY;j++){
				if(myDots[i][j].neighbors==3){
					myDots[i][j].exists=true;
					myDots[i][j].dotX=i;
					myDots[i][j].dotY=j;
					myDots[i][j].dotColor=defaultDotBodyColor;
		...
		for(int i=0;i<numDotX;i++){
			for(int j=0;j<numDotY;j++){
				if(myDots[i][j].neighbors>3 | myDots[i][j].neighbors<2){
					myDots[i][j].exists=false;
					myDots[i][j].dotX=i;
					myDots[i][j].dotY=j;
					myDots[i][j].dotColor=defaultBG;
					CanvFX.getGraphicsContext2D().setFill(myDots[i][j].dotColor);
					CanvFX.getGraphicsContext2D().fillRect(
							getAbsoluteDotPosX(i),
							getAbsoluteDotPosY(j),
	...
	*/	
	...
	@FXML
	public void onBut3(MouseEvent event) {
		// TODO Autogenerated
		// System.Out.println("But3Click");
		ShowGeneration();
		NextGen();
		//ClearGeneration();
		//		CanvFX.getGraphicsContext2D().clearRect(0, 0, CanvFX.getWidth(), CanvFX.getHeight());
